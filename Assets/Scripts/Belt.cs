...
// Author Class
public class BeltAuthoring : MonoBehaviour, IBuildingComponent
{
...
public struct BeltComponent : IComponentData, IBuildingComponent
{
    // Building Component Interface Variables.
    protected int[] acceptedResources;              // Belts are output-only; no inputs are accepted.
    protected readonly int maxStackSize;            // Non-belt building stack size assignment.
    protected readonly int totalInventorySize;      // 1 output slot = 1 * stack size.
    // Belt-specific Variables.
